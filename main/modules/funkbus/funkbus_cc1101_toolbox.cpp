...
// -----------------------------------------------------------------------------
// CC1101 register constants & wiring defaults
// -----------------------------------------------------------------------------
namespace cc1101 {
// Modem & packet format
...
// -----------------------------------------------------------------------------
// Persisted RF configuration (mirrors layout from commonRF/webUI)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// SPI one-liners (ELECHOUSE driver)
// -----------------------------------------------------------------------------
...
static inline void wB(uint8_t a, const uint8_t* p, size_t n) {
  ELECHOUSE_cc1101.SpiWriteBurstReg(a, (uint8_t*)p, n);
}
static inline void rB(uint8_t a, uint8_t* p, size_t n) {
  ELECHOUSE_cc1101.SpiReadBurstReg(a, p, n);
}
...
// -----------------------------------------------------------------------------
// Internal cached state & helpers (timing, calibration, comparisons)
// -----------------------------------------------------------------------------
namespace {
...
static uint32_t s_last_cal_khz = 0; // kHz to avoid float compares
...
inline bool scal_is_fresh(float mhz, uint32_t now_ms) {
  ...
  return (now_ms - s_last_cal_ms) < kScalMaxAgeMs;
}
...
// Run SCAL if frequency changed significantly or calibration window expired
...
// Tolerant MHz compare (~500 Hz)
inline bool same_mhz(float a, float b) { return fabsf(a - b) < 0.0005f; }
} // namespace

// -----------------------------------------------------------------------------
// Public API — basic state + MARC helpers
// -----------------------------------------------------------------------------
namespace FunkbusTB {
...
bool waitMarc(uint8_t target, uint32_t timeout_ms) {
...
uint8_t readMarc5() { return marc5_raw(); }

// Set center frequency in MHz; on large hops, force SCAL for stability.
void setMHz(float mhz) {
  if (s_has_current_mhz && same_mhz(mhz, s_current_mhz)) return;

  auto band_bucket = [](float fmhz) -> int {
    // CC1101 bands: 300–348, 387–464, 779–928 MHz
    if (fmhz < 380.0f) return 315; // 300–348
  ...
  if (big_hop) {
    st(cc1101::SIDLE);
    (void)waitMarc(MARC_IDLE, 2);
    st(cc1101::SCAL);
    (void)waitMarc(MARC_IDLE, 8);
#if FUNK_LOG_INFO
...
// -----------------------------------------------------------------------------
// Profile applier (masked writes + optional PATABLE; leaves radio IDLE)
// -----------------------------------------------------------------------------
bool FunkbusTB::applyProfile(const CC1101Profile& p) {
  ...
  // 1) IOCFG first (affects pin behavior)
  ...
  // 2) PKTCTRL0 masked update (async/whitening selection)
  if (p.pktctrl0_mask) {
    write_masked_if_diff(cc1101::PKTCTRL0, p.pktctrl0_mask, p.pktctrl0_value);
  }

  // 3) Core modem (OOK + data-rate + AGC)
  ...
  // 4) MCSM behavior (autocal/overflow) via masked writes
  ...
  // 6) No SCAL here; fast RX↔TX helpers handle it when needed.
  FB_LOG_N(F("[FunkbusTB] Profile applied (Δ writes only): %s" CR), p.name);
  return true;
}

// -----------------------------------------------------------------------------
// Persisted listen MHz (NVS) + cached helpers
// -----------------------------------------------------------------------------
...
void FunkbusTB::EnsureListenFreqInitialized() {
  ...
  FB_LOG_N(F("Listen MHz (cached) = %F (from FREQ regs: %u Hz)" CR),
           (double)s_listen_mhz, (unsigned)hz);
}

void FunkbusTB::SetListenMhz(float mhz) {
  if (!inFreqRange(mhz)) return; // silently ignore out-of-range
  s_listen_mhz = mhz; // cache only; OMG applies as needed
}
...
float FunkbusTB::GetPersistedMhz() {
  return RFConfig.frequency; // loaded from NVS at boot; updated via /rf Save
}
...
bool FunkbusTB::programFrequencyHz(uint32_t freq_hz, uint32_t fxosc_hz, bool do_calibrate) {
  ...
  FB_LOG_N(F("[FunkbusTB] Program FREQ=%u Hz -> read %u Hz (Δ=%ld Hz)" CR),
           (unsigned)freq_hz, (unsigned)rb,
           (long)((int32_t)rb - (int32_t)freq_hz));
  return true;
}
...
bool FunkbusTB::ensureDefaultFrequency(uint32_t tol_hz, uint32_t fxosc_hz) {
  ...
  if (d >= -(int32_t)tol_hz && d <= (int32_t)tol_hz) {
    FB_VLOG(F("[FunkbusTB] FREQ ok: %u Hz (Δ=%ld)" CR),
            (unsigned)now_hz, (long)d);
    return true;
  }
...
bool FunkbusTB::ensurePaTableOok(uint8_t offLevel, uint8_t onLevel, uint8_t onIndex) {
  ...
  if (!ok)
    FB_LOG_W(F("[FunkbusTB] PATABLE verify failed (on=%s idx=%u)" CR),
             hex8(onLevel), (unsigned)onIndex);
  return ok;
}

// -----------------------------------------------------------------------------
// RX/TX profile definitions
// -----------------------------------------------------------------------------
const CC1101Profile kProfile_Funkbus_RX = {
    ...
    .pktctrl0_value = cc1101::PKTCTRL0_ASYNC_VALUE,

    .mdmcfg2 = cc1101::MDMCFG2_OOK_NO_SYNC,
    .mdmcfg4_mask = cc1101::MDMCFG4_MASK,
    .mdmcfg4_value = cc1101::MDMCFG4_DRATE_E,
    .mdmcfg3 = cc1101::MDMCFG3_DRATE_M,

    .agcctrl2 = cc1101::AGCCTRL2_VAL,
    .agcctrl1 = cc1101::AGCCTRL1_VAL,
    .agcctrl0 = cc1101::AGCCTRL0_VAL,

    .deviatn = cc1101::REG_SKIP,
    .fsctrl1 = cc1101::REG_SKIP,

    .mcsm0_mask = cc1101::MCSM0_FS_AUTOCAL_MASK,
    .mcsm0_value = cc1101::MCSM0_RX_AUTOCAL,
    .mcsm1_mask = cc1101::MCSM1_RXOFF_MODE_MASK,
    .mcsm1_value = cc1101::MCSM1_RX_STAY_RX,

    .set_pa = true,
    .pa_off_level = cc1101::PA_OFF_LEVEL,
    .pa_on_level = cc1101::PA_ON_LEVEL,
    .pa_on_index = cc1101::PA_ON_INDEX,
};
...
const CC1101Profile kProfile_Funkbus_TX = {
    ...
    .mcsm0_mask = cc1101::MCSM0_FS_AUTOCAL_MASK,
    .mcsm0_value = cc1101::MCSM0_TX_NO_AUTOCAL, // we SCAL explicitly when needed
    .mcsm1_mask = cc1101::MCSM1_RXOFF_MODE_MASK,
    .mcsm1_value = cc1101::MCSM1_RX_STAY_RX,
...
bool FunkbusTB::assureRxReady(uint32_t timeout_ms) {
  (void)applyProfile(kProfile_Funkbus_RX); // leaves radio IDLE

  st(cc1101::SRX);
  if (waitMarc(MARC_RX, timeout_ms)) return true;

  // One quick recovery attempt (overflow or missed lock)
  goIdleAndFlush();
  st(cc1101::SCAL);
  (void)waitMarc(MARC_IDLE, 50);
  st(cc1101::SRX);
...
void FunkbusTB::readAllRegisters(uint8_t cfg[0x2F], uint8_t status[0x0C], uint8_t pa[8]) {
  ...
  for (uint8_t a = 0x30; a <= 0x3B; ++a)
    status[a - 0x30] = (uint8_t)ELECHOUSE_cc1101.SpiReadStatus(a);
...
static inline bool WaitTxReady() {
  ...
  while ((int32_t)(millis() - until) < 0) {
    ...
    if (st5 == FunkbusTB::MARC_TX || st5 == FunkbusTB::MARC_FSTXON) {
      FB_VLOG(F("CC1101 TX ready: %s (0x%s)" CR),
              FunkbusTB::marcName(st5), hex8(st5));
      return true;
    }
    vTaskDelay(pdMS_TO_TICKS(1));
  }
  FB_LOG_W(F("CC1101 TX not ready: %s (0x%s)" CR),
           FunkbusTB::marcName(st5), hex8(st5));
  return false;
}
...
bool FunkbusTB::switchRxToTxFast(uint32_t timeout_ms) {
  st(cc1101::SIDLE);
  if (!waitMarc(MARC_IDLE, 2)) return false;

  // Minimal diffs for TX (avoid full profile reapply)
  auto applyTxDiffs = []() {
    ...
    if (kProfile_Funkbus_TX.pktctrl0_mask) {
      ...
      v = (uint8_t)((v & ~kProfile_Funkbus_TX.pktctrl0_mask) |
                    (kProfile_Funkbus_TX.pktctrl0_value &
                     kProfile_Funkbus_TX.pktctrl0_mask));
      w8(cc1101::PKTCTRL0, v);
    }
  };
  applyTxDiffs();
  st(cc1101::STX);
  return waitMarc(MARC_TX, timeout_ms);
}
  ...
  auto applyRxDiffs = []() {
    ...
    if (kProfile_Funkbus_RX.pktctrl0_mask) {
      ...
      v = (uint8_t)((v & ~kProfile_Funkbus_RX.pktctrl0_mask) |
                    (kProfile_Funkbus_RX.pktctrl0_value &
                     kProfile_Funkbus_RX.pktctrl0_mask));
      w8(cc1101::PKTCTRL0, v);
    }
  };
  applyRxDiffs();
...
void FunkbusTB::beginTxSession() {
  ...
  // Try fast RX→TX; fallback to full prep if needed.
...
void FunkbusTB::endTxSession() {
  // Resume RX module; it performs SIDLE→SRX with minimal diffs.
...
bool FunkbusTB::rawOokBegin(RawTxSession& s, uint32_t tx_freq_hz) {
  ...
  // Optionally retune (with SCAL) for the TX session
  if (tx_freq_hz != 0 && tx_freq_hz != s.prev_freq_hz) {
    s.restore_freq = true;
    (void)programFrequencyHz(tx_freq_hz, FUNKBUS_CC1101_XTAL_HZ, true);
  }

  // Enter TX session (fast path applies OOK-pin semantics)
  beginTxSession();

  // Expect MARC_TX (or FSTXON briefly)
  ...
  return true; // caller now toggles FUNKBUS_CC1101_GDO0_MCU
}

bool FunkbusTB::rawOokEndRestore(const RawTxSession& s) {
  // Return to RX path and re-arm
  endTxSession();

  // Restore exact programmed frequency if we changed it
  if (s.restore_freq) {
    (void)programFrequencyHz(s.prev_freq_hz, FUNKBUS_CC1101_XTAL_HZ, true);
  }

  // Re-apply persisted listen MHz and ensure RX if we were in RX before
...
void FunkbusTB::selfTest_RxTxTransitions() {
  ...
  // Short OOK burst on GDO0 (sanity check; not a protocol TX path)
  beginTxSession();
#if defined(FUNKBUS_CC1101_GDO0_MCU) && (FUNKBUS_CC1101_GDO0_MCU >= 0)
  ...
  // Ensure we're back on listen frequency and RX-ready
  setMHz(GetListenMhz());
